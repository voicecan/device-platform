...
test('public Fastify API routes are represented in OpenAPI', async () => {
  const source = (await Promise.all(['app.ts', 'native-handoff.ts'].map(file => readFile(new URL(`../src/${file}`, import.meta.url), 'utf8')))).join('\n');
  ...
  routes.push('/native-handoffs/{id}/observe', '/native-handoffs/{id}/resume');
