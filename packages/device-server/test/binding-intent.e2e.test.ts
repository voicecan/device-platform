...
test('binding intent survives browser refresh and remains server-authoritative without a page callback', async (t) => {
  ...
  const history = await app.inject({ method: 'GET', url: '/api/v1/binding-intents?filter=unfinished&limit=10&offset=0', headers: adminHeaders });
  assert.equal(history.statusCode, 200, history.body);
  const entry = history.json().data.items.find((row: { id: string }) => row.id === intentId);
  assert.equal(entry.status, 'configured');
  assert.equal(entry.serial_number, 'INTENT-0001');
  assert.equal(entry.binding_path, 'web');
  for (const secret of ['browser_session_hash', 'launch_token_hash', 'device_token', 'token_ciphertext']) assert.equal(secret in entry, false);
  assert.equal((await app.inject({ method: 'GET', url: '/api/v1/binding-intents?limit=1000', headers: adminHeaders })).statusCode, 400);
  assert.equal((await app.inject({ method: 'GET', url: '/api/v1/binding-intents?offset=-1', headers: adminHeaders })).statusCode, 400);
  assert.equal((await app.inject({ method: 'GET', url: '/api/v1/binding-intents', headers: { cookie: browserCookie } })).statusCode, 401, 'single-task cookie cannot enumerate history');
  const reopenPath = `/api/v1/binding-intents/${intentId}/reopen`;
  assert.equal((await app.inject({ method: 'POST', url: reopenPath, headers: { cookie: adminHeaders.cookie, origin: 'http://127.0.0.1:8787' }, payload: {} })).statusCode, 403, 'reopen requires CSRF');
  assert.equal((await app.inject({ method: 'POST', url: reopenPath, headers: { ...adminHeaders, origin: 'http://evil.invalid' }, payload: {} })).statusCode, 403);
  const reopened = await app.inject({ method: 'POST', url: reopenPath, headers: { ...adminHeaders, origin: 'http://127.0.0.1:8787' }, payload: {} });
  assert.equal(reopened.statusCode, 200, reopened.body);
  assert.equal(reopened.json().data.id, intentId);
  const newCookie = String(reopened.headers['set-cookie']).split(';')[0]!;
  assert.notEqual(newCookie, browserCookie);
  const restored = await app.inject({ method: 'GET', url: `/api/v1/binding-intents/${intentId}/browser`, headers: { cookie: newCookie } });
  assert.equal(restored.statusCode, 200, restored.body);
  assert.equal(restored.json().data.provisioning_session_id, claim.json().data.provisioning_session_id);
  assert.equal(restored.json().data.device_id, claim.json().data.device_id);
  assert.equal(restored.json().data.status, 'configured');
  assert.equal(restored.json().data.expires_at, created.json().data.expires_at, 'reopening history does not extend device authorization');
  const outsider = await app.inject({ method: 'POST', url: '/api/v1/users', headers: adminHeaders, payload: { username: 'history-outsider', password: 'another long password', role: 'user' } });
  assert.equal(outsider.statusCode, 201);
  const outsiderLogin = await app.inject({ method: 'POST', url: '/api/v1/auth/login', payload: { username: 'history-outsider', password: 'another long password' } });
  const outsiderHeaders = { cookie: String(outsiderLogin.headers['set-cookie']).split(';')[0]!, 'x-csrf-token': outsiderLogin.json().data.csrf_token as string, origin: 'http://127.0.0.1:8787' };
  for (const target of [intentId, 'missing']) assert.equal((await app.inject({ method: 'POST', url: `/api/v1/binding-intents/${target}/reopen`, headers: outsiderHeaders, payload: {} })).statusCode, 404);
