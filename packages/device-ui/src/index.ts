...
export class VoicecanProvisionerElement extends LitElement {
  ...
  protected willUpdate(changed: PropertyValues<this>): void {
    ...
    const stateListener = (event: Event): void => {
      ...
      if (state === 'waiting_server' && this.step === 2 && this.selectedDevice) this.#startLiveStatus();
    };
  ...
  protected render() {
    ...
    return html`<section class=${this.compact && this.step === 0 ? 'card compact-status-card' : 'card'}>
