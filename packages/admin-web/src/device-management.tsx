  ...
  return <div className="device-workspace">
    ...
    {canRelease ? <section className="device-network-card">
      ...
      <div className="device-network-grid">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void configureServer(); }}><div className="form-title"><Icon name="server"/><div><h4>{t('Set device server address')}</h4><small>{t('Choose a detected LAN address or enter another reachable URL.')}</small></div></div><DeviceWsCandidatePicker candidates={serverCandidates} value={serverAddress} onChange={setServerAddress} t={t} canExportBackup={canRelease}/><Field id="maintenance-server-address" label={t('WebSocket server URL')} hint={t('Use an address reachable from the device network, for example a LAN IP or public domain.')} value={serverAddress} onChange={setServerAddress} placeholder="ws://192.168.0.180:8787/device/v1/ws" type="url" required/><Button disabled={!bleConnected || !serverAddress.trim() || Boolean(busy)}>{busy === 'server-configure' ? t('Updating…') : t('Set server address')}</Button></form>
      </div>
    </section> : null}
