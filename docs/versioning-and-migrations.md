...
This policy applies when the first stable release is cut. Preview versions may change, but every preview release must still publish its exact Core ABI, API version, schema version, and migration prerequisites.
...
- The private Core package uses SemVer plus an explicit `protocol_abi`, supported firmware range, conformance hash, and Browser/Node WASM digests. Device Server and Browser SDK accept only the pinned reviewed artifact.
...
- A new Core may be introduced only when both Browser and Node artifacts pass the same conformance set. The previous reviewed Core remains a rollback option for one stable Server minor line when the database/API contract remains compatible.
...
1. Stop or drain the instance, verify a fresh backup, and record application version, image/package digest, Core ABI/hash, and current schema version.
