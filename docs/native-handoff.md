...
Implemented 2026-09-08. This is the server/Web part of the native app plan (T07 and part of T08). Android/iOS task clients, trusted-instance configuration, verified App/Universal Links, QR UI and BLE task execution are still pending. The current discovery app does not execute these links. No production deployment or device acceptance is implied.
...
`cancel` stops future native mutations but leaves the provisioning session and device credential intact: a write may already have reached the device. Subsequent Gateway confirmation still wins. The response reports `stopped_pending_device_confirmation`, not device rollback. A cancelled task can still observe while its outer lifetime is valid. `owns_execution` is false for cancelled, superseded or lease-expired handoffs. Lost private-key recovery and a user-facing rebind after cancelled/expired tasks remain part of T08; never infer a safe device reset or generate a replacement Token.
